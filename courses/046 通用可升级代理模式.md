...
```
    代理合约：
        [ERC1967Proxy:Proxy]
            1. 使用高位存储槽，存储 newImplementation（即，逻辑合约地址）
            2. 与逻辑合约共享低位存储槽
            3. fallback 转发逻辑(指向[逻辑合约地址])
                    
    逻辑合约：
        [LogicV1:UUPSUpgradeable]
            1. 继承自 UUPSUpgradeable
            2. 升级逻辑（从proxy发起，fallback-delegate-implementation，最终把"逻辑合约地址"写入到proxy自身的高位槽）
            2. 定义变量与逻辑
            3. 自身存储槽，属于无用信息
